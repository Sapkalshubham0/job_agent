...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    welcome_msg = (
        "👋 <b>Welcome to the AI Job Hunter SaaS!</b>\n\n"
...
async def register(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("⚠️ Please provide location and job titles.\nExample: <code>/register Pune ; Data Analyst</code>", parse_mode='HTML')
        return

    user_input = " ".join(context.args)
    if ";" not in user_input:
        await update.message.reply_text("⚠️ Please separate your location and jobs with a semicolon (;).", parse_mode='HTML')
        return
    ...
    try:
        ...
        success_msg = (
            ...
            f"You will now receive automated matching updates!"
        )
        await update.message.reply_text(success_msg, parse_mode='HTML')
        
    except Exception as e:
        await update.message.reply_text(f"❌ An error occurred during registration.")
...
async def stop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🛑 Your job alerts have been paused.")
